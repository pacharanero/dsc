...
- [ ] **`dsc sar <discourse> <user>`** - one-shot Subject Access Request export: gather everything Discourse holds about one person (admin PII, authored posts, private messages, activity, groups) into a reviewable bundle with a cover sheet + `manifest.json` flagging what needs human review. GDPR Art. 15. Honest scope: automates data-gathering, not the legal judgement (identity, redaction, exemptions). Driver: NHS/medical-adjacent forums with real SAR obligations. Spec: [spec/subject-access-request.md](subject-access-request.md). Phase 1 = single forum.
